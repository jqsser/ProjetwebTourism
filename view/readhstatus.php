...
<svg style="display:none;">
  ...
  <section class="page-content">
    ...
    <input type="search" id="searchstatus" name="searchstatus" placeholder="Search Pages...">
...
include '../view/createhstatus.php';



$healthstatusC = new HealthstatusC();
$list = $healthstatusC->listhealthstatus();
/*if(isset($_POST['search'])) {
    $id_st = $_POST['id_st']; // Get id_st from form
    $healthstatus = $healthstatusC->getHealthStatusById($id_st); // Retrieve data based on id_st
} else {
    $list = $healthstatusC->listhealthstatus(); // Retrieve all data if search form is not submitted
}*/
$d = new HealthstatusC(NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
$tab = array();

// Vérifier si le formulaire de recherche est soumis
if (isset($_GET["searchstatus"])) {
  // Récupérer la valeur de recherche
  $search_query = $_GET["searchstatus"];

  // Effectuer la recherche
  $tab = $d->recherchestatus($search_query);} 
  else {
    $tab = $d->showstatus();
}
?>
...
<body>
<div class="container">
<a href="readassur.php" class="move-left-top" style="color: blue;">Back to list of client's assurance </a>
<div class="center">
    
        <h1 class="page-title">List of client's health status</h1>
        <td> <img src="healthstatus.jpg" alt ="image" width="5%" class="image-position" /> </td>
        <h2>
        <a class="btn btn-blue move-right" onclick="togglePopup()">Add health status</a>
    </h2>
        <br>
    </br>
        </div>
    <table border="1" align="center" width="70%">
        <tr>
            ...
            <th>exercise</th>
            <th>oldpeak</th>
            <th>heart rate slope</th>
            <th>heart disease</th>
            <th>health score</th>
            <th>actions</th>
        </tr>
        <?php
        foreach ($list as $healthstatus) {
        ?>
            <tr>
                <td><?= $healthstatus['id_st']; ?></td>
                <td><?= $healthstatus['age']; ?></td>
                <td><?= $healthstatus['gender']; ?></td>
                <td><?= $healthstatus['chestpain_type']; ?></td>
                <td><?= $healthstatus['resting_bp']; ?></td>
                <td><?= $healthstatus['cholesterol']; ?></td>
                <td><?= $healthstatus['fastingBS']; ?></td>
                <td><?= $healthstatus['normalESG']; ?></td>
                <td><?= $healthstatus['maxHR']; ?></td>
                <td><?= $healthstatus['exercise']; ?></td>
                <td><?= $healthstatus['oldpeak']; ?></td>
                <td><?= $healthstatus['ST_slope']; ?></td>
                <td><?= $healthstatus['Heart_disease']; ?></td>
                <td><?= $healthstatus['health']; ?></td>
                <td align="center">
                <form method="POST" action="modifyhstatus.php">
                        
                        <button class="btn btn-blue"  onclick="PopupUpdateSPForm('<?= $rec['id_st'] ?>', '<?= $rec['age'] ?>', '<?= $rec['gender'] ?>', '<?= $rec['chestpain_type'] ?>', '<?= $rec['resting_bp'] ?>', '<?= $rec['cholesterol'] ?>', '<?= $rec['fastingBS'] ?>', '<?= $rec['normalESG'] ?>', '<?= $rec['maxHR'] ?>', '<?= $rec['exercise'] ?>', '<?= $rec['oldpeak'] ?>', '<?= $rec['ST_slope'] ?>', '<?= $rec['Heart_disease'] ?>', '<?= $rec['health'] ?>')">Update</button>
                        <input type="hidden" value=<?PHP echo $healthstatus['id_st']; ?> name="id_st">
                        <br></br>
                        <a href="deletehstatus.php? id_st=<?php echo $healthstatus['id_st']; ?>" onclick="return confirm('Are you sure you want to delete this health status?')" style="background-color: #ccc; color: #fff; padding: 5px 10px; text-decoration: none; border-radius: 5px;">Delete</a>
