...
<svg style="display:none;">
  ...
  <section class="page-content">
    ...
    <section class="search-and-user">
      <form id="searchForm" method="GET" action="searchassur.php">
        <input type="search" id="search_id" name="search_id" placeholder="Search Pages...">
...
include '../view/createassur.php';
$assuranceC = new AsrclientC();
$list = $assuranceC->listassurance();
$assuranceC = new AsrclientC();

// Check if the form is submitted with an ID to search for
if(isset($_GET['search_id'])) {
    $search_id = $_GET['search_id'];
    $assurance = $assuranceC->searchAssuranceById($search_id);

    // If assurance is found, display details in pop-up
    if ($assurance) {
        $assurance_details = json_encode($assurance);
        echo "<script>displayAssurancePopup($assurance_details);</script>";
    } else {
        // If no assurance is found, show an alert
        echo "<script>alert('No assurance found with ID: $search_id');</script>";
    }
}
...
<head>
    ...
    <title>List of Assurances</title>
    ...
    color: black; /* Set text color to black */
        ...
        .move-right {
    float: right ;
    margin-top: 35px; /* This will float the button to the right */
}
...
.page-title {
    ...
    margin-top: -20px; /* Add some top margin to push it down from the top */
}
.image-position {
  position: relative;
  top: -70px;
...
<body>
    <div class="container">
        <h1 class="page-title">List of client's assurances</h1>
        <div class="center">
        <td> <img src="client.png" alt ="image" width="5%" class="image-position" /> </td>
        <a href="readhstatus.php" class="move-left-top" style="color: blue;" >List of Health Status</a>

            <a class="btn btn-blue move-right" onclick="toggleAjoutSPPopup()" >Add Assurance</a>
        ...
        <table border="1" width="100%" >
            ...
            <?php foreach ($list as $assurance) : ?>
                <tr>
                    ...
                    <td><?= $assurance['heaid']; ?></td>
                    <td>
                    <button class="btn btn-blue" onclick="toggleUpdateSPPopup()" onclick="PopupUpdateSPForm('<?= $rec['id_assur'] ?>', '<?= $rec['type_assur'] ?>', '<?= $rec['datedebut'] ?>', '<?= $rec['datefin'] ?>', '<?= $rec['clienthealth'] ?>','<?= $rec['asrprovider'] ?>','<?= $rec['cost'] ?>','<?= $rec['type'] ?>','<?= $rec['heaid'] ?>')">Update</button>
                    </td>
                    <td>
                        <a href="deleteassur.php?id_assur=<?= $assurance['id_assur']; ?>" class="btn btn-delete" onclick="return confirm('Are you sure you want to delete this assurance?')" style="background-color: #ccc; color: #fff; padding: 5px 10px; text-decoration: none; border-radius: 5px;">Delete</a>
                    </td>
                </tr>
                <tr id="searchedAssuranceRow" style="display: none;">
        <!-- Data for searched assurance will be inserted here -->
    </tr>
            <?php endforeach; ?>
        </table>
    </div>
    ...
    <h2>Update assurance</h2>
    <form action="updateassur.php" method="POST" onsubmit="return validateFormSpaceship()">
            <div class="form-group">
                        <label for="id_assur">id d'assurance':
                        </label>
                    
                    <input type="number" name="id_assur" id="id_assurup" value="<?php echo $assurance['id_assur']; ?>" >
                        </div>
                        <div class="form-group">
                    ...
                    <input type="text" name="type_assur" id="type_assurup" value="<?php echo $assurance['type_assur']; ?>" >
                        </div>
                        <div class="form-group">
                        <label for="datedebut">date debut:
                        </label>
                    
                    <input type="date" name="datedebut" id="datedebutup" value="<?php echo $assurance['datedebut']; ?>" >
                        </div>
                        <div class="form-group">
                        <label for="datefin">date fin:
                        </label>
                    
                    <input type="date" name="datefin" id="datefinup" value="<?php echo $assurance['datefin']; ?>" >
                        </div>
                        <div class="form-group">
                        <label for="clienthealth">client health:
                        </label>
                    
                    <input type="number" name="clienthealth" id="clienthealthup" value="<?php echo $assurance['clienthealth']; ?>" >
                        </div>
                        <div class="form-group">
                        <label for="asrprovider">assurance provider:
                        </label>
                    
                    <input type="text" name="asrprovider" id="asrproviderup" value="<?php echo $assurance['asrprovider']; ?>" >
                        </div>
                        <div class="form-group">
                    ...
                    <input type="number" name="cost" id="costup" value="<?php echo $assurance['cost']; ?>" >
                        </div>
                        <div class="form-group">
                        <label for="type">type:
                        </label>
                    
                    <input type="text" name="type" id="typeup" onblur="verifiertypeassurance()" value="<?php echo $assurance['type']; ?>" >
                    <span id="typMessage" style="color: red;"></span>
                        </div>
                        <div class="form-group">
                        <label for="heaid">health id:
                        </label>
                    
                    <input type="number" name="heaid" id="heaidup" value="<?php echo $assurance['heaid']; ?>" >
                        </div>
    ...
    <script>
        ...
        function PopupUpdateSPForm(id_assur, type_assur, datedebut, datefin,clienthealth,asrprovider,cost,type,heaid) {
            document.getElementById("id_assurup").value = id_assur;
            document.getElementById("type_assurup").value = type_assur;
            document.getElementById("datedebutup").value = datedebut;
            document.getElementById("datefinup").value = datefin;
            document.getElementById("clienthealthup").value = clienthealth;
            document.getElementById("asrproviderup").value = asrprovider;
            document.getElementById("costup").value = cost;
            document.getElementById("typeup").value = type;
            document.getElementById("heaidup").value = heaid;
    ...
    <script>
   // Function to handle AJAX response and update table
function handleSearchResult(assurance) {
    if (assurance.id_assur) {
        // Get the rows
        var searchedAssuranceRow = document.getElementById("searchedAssuranceRow");
        var regularAssuranceRow = document.querySelector("table tr:not(#searchedAssuranceRow)");

        // Swap the HTML content of the two rows
        var tempHTML = regularAssuranceRow.innerHTML;
        regularAssuranceRow.innerHTML = searchedAssuranceRow.innerHTML;
        searchedAssuranceRow.innerHTML = tempHTML;

        // Make the second row visible
        searchedAssuranceRow.style.display = "";
    } else {
        // Display a message if assurance not found
        alert("No assurance found with the provided ID.");
    }
}

// Submit the form via AJAX to fetch assurance details
document.getElementById("searchForm").addEventListener("submit", function(event) {
    event.preventDefault(); // Prevent default form submission
    var searchId = document.getElementById("search_id").value;

    // AJAX request to fetch assurance details
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                // If successful, handle the search result
                handleSearchResult(JSON.parse(xhr.responseText));
            } else {
                // If an error occurred, display an alert
                alert("Error fetching assurance details.");
            }
        }
    };
    xhr.open("GET", "searchassur.php?search_id=" + searchId, true);
    xhr.send();
});
