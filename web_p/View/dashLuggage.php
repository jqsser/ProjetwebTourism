...
include './addLuggage.php';

$d = new LuggageC(NULL,NULL,NULL);
            ...
            <tbody>
                <?php foreach ($tab as $rec) { ?>
                    <tr>
                        ...
                        <td><?= $rec['weight_Lu'] ?></td>
                        <td align="center">
                            <button class="btn btn-blue" onclick="PopupUpdateLugForm('<?= $rec['id_luggage'] ?>', '<?= $rec['type_Lu'] ?>', '<?= $rec['weight_Lu'] ?>')">Update</button>
                            <form id="deleteForm<?= $rec['id_luggage'] ?>" action="deleteLuggage.php" method="POST">
                                <input type="hidden" name="id_luggage" value="<?= $rec['id_luggage'] ?>">
                                <button type="button" class="btn btn-delete" onclick="confirmDelete(<?= $rec['id_luggage'] ?>)">Delete</button>
                            </form>
                        </td>
                    </tr>
                <?php } ?>
...
<div id="updateLugPopup" class="popup">
    ...
    function toggleUpdateLugPopup() {
        var popup = document.getElementById("updateLugPopup");
        popup.style.display = popup.style.display === "none" ? "block" : "none";
    }

    function PopupUpdateLugForm(id_luggage, type_Lu, weight_Lu) {
        document.getElementById("update_id_luggage").value = id_luggage;
        document.getElementById("update_type_Lu").value = type_Lu;
        document.getElementById("update_weight_Lu").value = weight_Lu;
        toggleUpdateLugPopup(); // Show
    }

    function validateUpdateLugForm() {
        var type_Lu = document.getElementById("update_type_Lu").value;
        var weight_Lu = document.getElementById("update_weight_Lu").value;
        
        if (type_Lu.trim() === "" || weight_Lu.trim() === "") {
