...
$commentcr  = new commentcr(NULL,NULL,NULL,NULL);

    $tab = $commentcr  ->show_commentr();
    ...
    <table>
        ...
        <tbody>
            <?php foreach ($tab as $rec) { ?>
                <tr>
                    <td><?= $rec['id_cr'] ?></td>
                    <td><?= $rec['content_cr'] ?></td>
                    <td><?= date('d/m/Y', strtotime($rec['date_cr'])) ?></td>
                    <td><?= $rec['rating_cr'] ?></td>
                    <td>
               
                        <form action="update_commentR.php" method="POST">
                            <input type="hidden" name="id_cr" value="<?= $rec['id_cr'] ?>">
                            <button type="submit" class="btn btn-outline-success btn-sm">Modifier</button>
                        </form>
                        <form action="delete_commentR.php" method="POST">
                            <input type="hidden" name="id_cr" value="<?= $rec['id_cr'] ?>">
